...
struct FileInstrument {
	...
	{
		if (statement.getSymbol() == ts_symbol_identifiers::sym_compound_statement)
			return instrumentCompoundStatement(statement);
		...
		if (statement.getNumChildren() > 0)
			instrumentPossibleOneLiner(statement);
	}
		...
		for (const auto& topLevelExpr : ts::Children(tree.getRootNode()))
		{
			if (topLevelExpr.getSymbol() == ts_symbol_identifiers::sym_function_definition)
			{
				...
				for (const auto& child : ts::Children(topLevelExpr))
				{
					if (child.getSymbol() == ts_symbol_identifiers::sym_compound_statement)
					{
						instrumentCompoundStatement(child);
						break;
					}
				}
				//std::cout << std::endl;
